    ...
    public class Part02
    {
        ...
        private Dictionary<string, Dictionary<char, long>> FindPairsOfDigits()
        {
            ...
            for (int hand = 0; hand < hands.Count; hand++)
            {
                ...
                if(pair.Count == 0 && item.Key.Contains('J')) { pair.Add('J', 1); }

                pair = UpdatePairsIfItHasJoker(item.Key, pair);
        ...
        private Dictionary<char, long> UpdatePairsIfItHasJoker(string item, Dictionary<char, long> pair)
        {
            if (item.Contains('J'))
            {
                if (pair.Count == 1)
                {
                    var p = pair.ElementAt(0);
                    if(p.Value != 5)
                    {
                        pair[p.Key] = p.Value + 1;
                    }
                }
                else
                {
                    if (pair.Count == 2)
                    {
                        var item0 = pair.ElementAt(0);
                        var item1 = pair.ElementAt(1);

                        if (pair.ContainsKey('J'))
                        {
                            if (item0.Value != item1.Value)
                            {
                                pair.Remove(item1.Key);
                                pair[item0.Key] = 5;
                            }
                            else
                            {
